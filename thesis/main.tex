...
Five contributions are made. First, the batch scheduling problem is formulated as a two-machine flow-shop in which Machine~A represents I/O (loading model weights) and Machine~B represents GPU computation, enabling Johnson's Rule for provably optimal model group ordering. Second, a shared-aware scheduling algorithm discovers model-sharing relationships within a batch, groups tasks by model to reduce switches from $O(n)$ to $O(k)$, and orders groups by Johnson's Rule to maximise I/O-computation overlap. Third, a checkpoint prefetching mechanism uses the pinned memory pool of ServerlessLLM to preload the next model's weights while the current group executes, hiding I/O latency through pipelining. Fourth, the prefetch architecture is analysed and evaluated across storage tiers including RAID-0, 10\,GbE NFS, and 100\,GbE NFS, demonstrating that it generalises to network-attached storage. Fifth, an OpenAI-compatible batch REST API is extended for multi-model batches, enabling clients to submit heterogeneous task lists in a single request and receive results asynchronously.
