...
0.6B$\to$32B$\to$8B           & 132.9 $\pm$ 4.4 & 2/6 & +0.4\% & 0.6B-first \\
8B$\to$32B$\to$0.6B           & 173.0 $\pm$ 2.8 & 3/6 & +30.7\% & 8B-first \\
...
Table~\ref{tab:exp4_sensitivity} shows the results across $n=5$, 15, and 30. At $n=5$, the JR ordering (0.6B$\to$32B$\to$8B) achieves rank~2/6, 4.7s behind the empirical fastest; this gap is in the predicted direction but does not reach statistical significance ($t\approx2.2$, $p>0.05$ with 4\,d.f.). At $n=15$, the gap between the two 0.6B-first orderings widens to 7.3s, consistent with a gradual transition. By $n=30$, all groups become compute-dominant again and the gap collapses to 0.5s.

\begin{table}[htbp]
\centering
\caption{Task-count sensitivity: makespan (mean\,$\pm$\,std, seconds) across three batch sizes. Bold: JR-predicted optimal for that regime.}
...
\textbf{0.6B$\to$32B$\to$8B} $\ddagger$ & \textbf{135.8$\pm$3.0} & 145.2$\pm$11.6 & 132.9$\pm$4.4 \\
...
\bottomrule
\end{tabular}
\footnotesize{$\dagger$\,JR-optimal at $n\geq15$; $\ddagger$\,JR-optimal at $n=5$ (8B and 32B I/O-dominant). ``---'': not measured at $n=15$.}
\end{table}
