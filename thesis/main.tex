...
The speedup grows rapidly from 65$\times$ at 99 tasks to 276$\times$ at 501 tasks, and continues to grow with batch size as FIFO's $O(n)$ switch cost increasingly dominates. For FIFO with fully-interleaved submission across $k=3$ models (average switch cost $\bar{L}_{switch} = 90.3$s from cold-start sampling):
...
The results reveal a clear three-tier structure. Both 0.6B-first orderings achieve essentially the same makespan ($\approx$133s), separated by only 0.5s --- within the $\pm$3.5s measurement noise of the faster run. The two 8B-first orderings average 174.4s (+31.7\% vs JR), and the two 32B-first orderings average 225.5s (+70.3\% vs JR). The spread across all six permutations is 95.4s.
...
H3 is confirmed: Johnson's Rule ranks 1/6, achieving the empirical optimum. The ordering spread (95.4s, 72.1\% from best to worst) demonstrates that group order has first-order impact on makespan, and JR's deterministic selection eliminates this uncertainty.

\subsection{Regime Sensitivity: Task-Count Variation}

The tie between the two 0.6B-first orderings arises because, at $n=30$ tasks per model, both subsequent models are fully staged in CPU memory before the 0.6B group finishes: with eager prefetch starting at batch submission, the combined SSD transfer for 8B (5.1s) and 32B (21.7s) completes in $\approx$26.8s, well within the 0.6B group's $\approx$45s elapsed time. Their relative execution order therefore cannot affect makespan. To test whether JR's S2 sub-ranking becomes physically meaningful at shorter task counts---where the smaller compute window may no longer fully cover the larger SSD loads---all six permutations were re-run at $n=5$ tasks per model, and the two 0.6B-first orderings were additionally measured at $n=15$.

At $n=5$, each model group executes only $\approx$5 tasks ($\approx$2--3s compute). With SSD-only $a_i$, the 8B and 32B groups become I/O-dominant (S2): $b_{8B}\approx2.5$s\,$<$\,$a_{8B}=5.1$s and $b_{32B}\approx6$s\,$<$\,$a_{32B}=21.7$s. The 0.6B group remains S1. JR adapts: S1 (0.6B) first, then S2 sorted by $b_i$ descending (32B before 8B), prescribing 0.6B$\to$32B$\to$8B.
...
JR's ordering rule adapts correctly to workload regime. At $n=30$, all groups are compute-dominant (S1) and JR prescribes 0.6B$\to$8B$\to$32B, achieving rank~1/6. At $n=5$, shorter task windows make 8B and 32B I/O-dominant (S2), so JR adapts to 0.6B$\to$32B$\to$8B, achieving rank~2/6 with a 4.7s gap in the predicted direction (the difference does not reach statistical significance at $n=3$ runs per condition). In both regimes the three-tier structure (0.6B-first $\ll$ 8B-first $\ll$ 32B-first) is the dominant pattern, and JR deterministically selects the correct first-position model without runtime enumeration. At $n=5$, the worst ordering (32B$\to$0.6B$\to$8B, 219.4s) is 67\% slower than the best 0.6B-first ordering; the 88.3s spread across all six orderings confirms that group order has first-order impact regardless of task count.
...
Yes. The problem maps to a two-machine flow-shop (Machine~A: I/O, Machine~B: GPU). Prefetching implements the I/O-compute overlap, reducing makespan by 36.6\% and hiding approximately half of cold-switch latency (Experiment~2). Johnson's Rule ranks 1/6 (empirical optimum) among all $3!=6$ permutations, with 32B-first orderings averaging 70.3\% slower and 8B-first orderings averaging 31.7\% slower, and the ordering spread across all permutations is 95.4s (Experiment~4).

\paragraph{RQ3: How Do Strategies Compare?}

FIFO is unsuitable (over 99\% of time on loading). Model Grouping gives deterministic first-order improvement (199$\times$). Shared-Aware adds I/O-computation overlap, with speedup growing from 65$\times$ to 276$\times$ across measured batch sizes (Experiment~3a). Johnson's Rule ranks 1/6 (empirical optimum) among all 6 permutations, avoiding the 32B-first penalty ($\sim$70\%) and the 8B-first penalty ($\sim$32\%), with the full permutation spread demonstrating 95.4s ordering sensitivity (Experiment~4).
...
Experiment~1 shows any model-aware strategy dramatically outperforms FIFO (164.7s measured vs 45,495s extrapolated for 501 tasks, a 276$\times$ speedup), justifying the semaphore concurrency approach. The semaphore buffer size (default 10) trades GPU utilisation against memory pressure.
...
In the schematic diagram (Figure~\ref{fig:exp4_ordering_gantt}), using stylised processing times ($b_1=9$\,s, $b_2=36$\,s, $b_3=15$\,s), the lower bound evaluates to $\max(60,\; 0.4+60,\; 82.8+15) = 60.4$\,s. Johnson's Rule achieves 73.1\,s in the schematic (21.0\% above the schematic lower bound); the gap arises because the 32B model's 21.7\,s load exceeds the 0.6B group's 9\,s compute window, leaving a 12.7\,s residual that no ordering can eliminate. In the actual experiment, the JR ordering achieved \textbf{132.4\,s} (Table~\ref{tab:exp4_ordering_results}); the additional overhead relative to the schematic reflects vLLM initialisation costs ($\sim$40--60\,s per transition) that are not modelled in the two-machine flow-shop formulation but remain constant across orderings and therefore do not affect the relative ranking of orderings.
