...
% ── Contribution legend ──
\node[draw=black!40, fill=cbluelight, minimum width=0.45cm, minimum height=0.28cm, inner sep=0, anchor=east]
    (ll_e) at ($(ssd.south) + (-0.6, -0.75)$) {};
\node[font=\scriptsize, anchor=west] at (ll_e.east) {\;Existing};
\node[draw=cpurple!70, fill=cpurplelight, minimum width=0.45cm, minimum height=0.28cm, inner sep=0, anchor=west]
    (ll_x) at ($(ll_e.east) + (1.6, 0)$) {};
...
\textbf{Models Tested.}
All experiments use three Qwen models spanning a wide range of checkpoint sizes: Qwen3-0.6B ($\sim$1.2\,GB, $a=0.4$\,s), Qwen3-8B ($\sim$15.4\,GB, $a=5.1$\,s), and Qwen2.5-32B-Instruct ($\sim$65\,GB, $a=21.7$\,s), where $a$ is the NVMe SSD load time. The 0.6B and 8B models each run on a single GPU; the 32B model exceeds single-GPU VRAM (24\,GB) and runs with tensor parallelism across 4 GPUs (TP=4), which the scheduler detects and allocates automatically.
...
\caption{Incremental scheduling strategy ablation for 501 tasks with 3 models (0.6B, 8B, 32B). FIFO ($\approx$12.6h, computed from direct cold-start measurements) is shown on a separate axis. Each column adds one optimisation component.}
\label{fig:exp1_bar}
\end{figure}
