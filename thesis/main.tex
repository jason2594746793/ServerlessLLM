...
Evaluation on NVIDIA RTX A5000 GPUs shows that, for a 501-task batch spanning three models (0.6B--32B), model grouping reduces makespan by over 99\% versus FIFO (FIFO equivalent runtime: 45{,}495\,s, computed from direct cold-start measurements; model grouping measured: 228.5\,s). Checkpoint prefetching hides the SSD I/O portion of model transitions, reducing makespan by a further $\sim$22\,s (9.5\%). Johnson's Rule ordering adds a further 20\% by placing model groups optimally for prefetch overlap, yielding a combined 276$\times$ speedup. The shared-aware strategy outperforms FIFO by 65--276$\times$ across measured batch sizes of 99--501 tasks and model sizes of 0.6B--32B, with speedup growing as batch size increases. Prefetching remains effective across storage tiers including 10\,GbE network storage, where 78.9\% of I/O read latency is analytically estimated to be hidden, analytically projecting the viability of remote storage in production deployments.
}
...
\textbf{RQ1:} How can shared structure (shared model weights) in a batch be discovered and exploited to reduce makespan, and how does model grouping compare to naive FIFO scheduling?

\textbf{RQ2:} Can the batch scheduling problem be modelled as a two-machine flow-shop of I/O and compute stages, and does Johnson's Rule provide an effective group ordering to maximise I/O-computation overlap via checkpoint prefetching?

\textbf{RQ3:} How do FIFO, model grouping, and shared-aware scheduling compare across varying batch sizes, model sizes, and storage tiers (local SSD, RAID, network storage)?

\section{Contributions}

Five contributions are made. The \textbf{two-machine flow-shop formulation} maps model loading (I/O) and GPU inference (compute) to Machine~A and Machine~B respectively, providing the theoretical justification for applying Johnson's Rule. The \textbf{shared-aware scheduling algorithm} groups tasks by model, reducing switches from $O(n)$ to $O(k)$, then applies the flow-shop formulation to determine the optimal group execution order and trigger prefetching. The \textbf{checkpoint prefetching mechanism} uses the ServerlessLLM pinned memory pool to load the next model's checkpoint from SSD into CPU memory during the current group's execution, hiding the I/O cost of each transition. The \textbf{storage tier characterisation} evaluates prefetching effectiveness across NVMe, RAID-0, 10\,GbE NFS, and 100\,GbE NFS, quantifying how storage bandwidth affects the overlap window and residual transition cost. Finally, an \textbf{OpenAI-compatible batch API} extends the ServerlessLLM API to accept multi-model batch submissions in OpenAI Batch API format, enabling drop-in migration from cloud batch services.

The project contributed approximately 3000 lines of new core framework Python code to the existing ServerlessLLM framework, with additional benchmark scripts and unit tests on top of that.

\section{Scope and Boundaries}

This work focuses strictly on cluster-level scheduling of \emph{offline} batches: all tasks are submitted together as a single JSONL file and the complete task-to-model mapping is known before scheduling begins. Dynamic batching, where tasks arrive continuously during execution, is not considered; the scheduling decisions (grouping, ordering, prefetch triggering) rely on global knowledge of the batch and would require redesign for an online arrival model. The inference engine is neither modified nor optimised; KV cache management, prefill-decode separation, continuous batching internals, and GPU kernel optimisations are out of scope. The scheduler decides which tasks to execute, in what order, and on which workers; vLLM handles the computation.  
...
LLMs are built on the transformer architecture \citep{vaswani2017attention}, generating tokens autoregressively via a compute-bound prefill phase ($O(L^2)$ attention) followed by a memory-bandwidth-bound decode phase (one token per step). Modern LLMs span from under 1B to hundreds of billions of parameters, with the landscape expanding rapidly; an 8B model requires $\sim$15\,GB in FP16 and a 70B model $\sim$140\,GB.

vLLM \citep{kwon2023efficient} introduced continuous batching (iteration-level scheduling), inserting new requests at every decode step to maintain near-full GPU utilisation within a single loaded model. vLLM does not address switching between models. Tensor parallelism \citep{shoeybi2019megatron} shards weight matrices across multiple GPUs for models exceeding the VRAM of a single GPU; the proposed scheduler automatically detects when a model requires tensor parallelism and allocates a dedicated multi-GPU lane accordingly.

\section{KV Cache and Prefix Caching}
\label{sec:bg_kv_cache}

Inference engines cache key-value tensors to avoid redundant computation. vLLM manages the KV cache in fixed-size blocks via PagedAttention \citep{kwon2023efficient}, eliminating fragmentation. When requests share a prompt prefix, KV blocks can be computed once and reused \citep{zheng2024sglang}, though interleaving different prefixes causes eviction. Prefix caching is enabled by default in vLLM, so tasks within a group already benefit from cache hits whenever their prompts share a common prefix. What the scheduler does not currently do is \emph{reorder tasks within a group} to maximise the cache hit rate; tasks are dispatched in submission order, which may cause evictions when different prefixes interleave. Prefix-aware intra-group ordering is identified as future work (Chapter~\ref{ch:future}).
...
\begin{enumerate}
  \item \textbf{Remote storage $\to$ Local SSD} (optional): When models are not locally cached, checkpoint files are downloaded from object stores (S3, GCS) or network filesystems (NFS). Bandwidth varies significantly by storage tier and is discussed in Section~\ref{sec:bg_storage_hierarchy}.
  \item \textbf{Local SSD $\to$ CPU RAM}: Checkpoint files are read via filesystem I/O into host memory. Bandwidth: 2--5\,GB/s on NVMe SSD.
  \item \textbf{CPU RAM $\to$ GPU VRAM}: Weights are transferred via PCIe DMA. Bandwidth: 16--32\,GB/s on PCIe Gen4 x16. This step is faster with \emph{pinned} (page-locked) memory allocated via \texttt{cudaHostAlloc} or \texttt{cudaHostRegister} \citep{cuda_guide2024}, which enables direct DMA without intermediate copies.
\end{enumerate}

In deployments where models are cached on local SSD, only stages 2 and 3 are on the critical path. The sllm-store component of ServerlessLLM \citep{fu2024serverlessllm} pre-allocates a pinned memory pool at startup; weights are loaded into this pool via multi-threaded I/O and transferred to the GPU via DMA, achieving near-peak PCIe bandwidth.
...
ServerlessLLM \citep{fu2024serverlessllm} is an open-source serverless LLM inference framework designed for low-latency model loading. The \textbf{Head Node} receives inference requests via an API gateway, maintains deployment state in a SQLite database, and dispatches tasks to workers via a Router. \textbf{Workers} (Pylets) are GPU nodes managed by the Pylet process manager; each runs one or more vLLM \citep{kwon2023efficient} instances for inference, with Pylet handling instance lifecycle and reporting resource availability. On each worker, \textbf{sllm-store} is a C++ storage daemon that manages a pinned memory pool and provides gRPC endpoints for model loading \citep{grpc2024}, using multi-threaded I/O to saturate SSD bandwidth; its \texttt{LoadModelAsync} operation loads weights into pinned CPU memory for subsequent DMA transfer. The \textbf{AutoScaler} monitors deployment state and adjusts vLLM instance counts reactively, scaling up when queue depth exceeds a threshold and scaling down after an idle timeout.
...
The system extends the ServerlessLLM head node with three new components: the BatchScheduler (model grouping and Johnson's Rule ordering), the StorageManager (checkpoint prefetching into the pinned memory pool), and a batch-aware AutoScaler integration, all operating at cluster scheduler level without inference engine modifications. A SQLite database (\texttt{state.db}) stores batch jobs, task status, and deployment configuration; the Router reads healthy worker endpoints from it. The user submits a batch via the API Gateway; the BatchScheduler groups tasks, applies Johnson's Rule, persists job state, and feeds ordered tasks to the Router, which dispatches them to vLLM workers. Concurrently, the StorageManager prefetches the next model's weights while the current group executes, and the AutoScaler adjusts instance count as needed.
...
\begin{tikzpicture}[
    ...
    newbox/.style={box, fill=camber!30, draw=camber!80!black},
    extbox/.style={box, fill=cpurplelight, draw=cpurple!70},
]
...
\node[extbox, below=1.4cm of user] (gateway) {API Gateway};

% Row 2: scheduler (left) and storage_mgr (right), symmetric about x=0
\node[newbox, below=1.1cm of gateway, xshift=-2.5cm] (scheduler) {BatchScheduler};
\node[extbox, right=2.0cm of scheduler]              (storage_mgr) {StorageManager};

% Row 3: router under scheduler, db under storage_mgr
\node[extbox, below=1.1cm of scheduler] (router) {Router};
\node[extbox,  below=1.1cm of storage_mgr, draw=cpurple!70] (db) {SQLite DB\\{\scriptsize state.db}};
...
% ── Contribution legend ──
\node[draw=cpurple!70, fill=cpurplelight, minimum width=0.45cm, minimum height=0.28cm, inner sep=0, anchor=east]
    (ll_x) at ($(ssd.south) + (-0.6, -0.75)$) {};
\node[font=\scriptsize, anchor=west] at (ll_x.east) {\;Extended (this work)};
\node[draw=camber!80!black, fill=camber!30, minimum width=0.45cm, minimum height=0.28cm, inner sep=0, anchor=west]
    (ll_n) at ($(ll_x.east) + (2.8, 0)$) {};
\node[font=\scriptsize, anchor=west] at (ll_n.east) {\;New (this work)};
...
Two sharing relationships exist within a batch. Model sharing arises when $m_i = m_j$: tasks $t_i$ and $t_j$ can share loaded model weights, avoiding redundant loading; this is the primary relationship exploited in this work. Prefix sharing arises when $\text{prefix}(p_i) = \text{prefix}(p_j)$, enabling inference engine cache reuse; vLLM's built-in prefix caching handles this passively, but the scheduler does not reorder tasks to maximise the cache hit rate (see Chapter~\ref{ch:future}).
...
The formal problem definition above establishes two sharing relationships. Model sharing has first-order impact: since $L_{load}$ is 100--200$\times$ larger than $L_{infer}$ (Section~\ref{sec:bg_serverlessllm}), eliminating switches from $O(n)$ to $O(k)$ dominates all other optimisations. Prefix sharing is a second-order effect: vLLM's built-in prefix caching already reduces prefill time from $\sim$2\,s to $\sim$0.05\,s per task for cache hits, which is negligible compared to the 80--120\,s model-switch cost. What is deferred to future work is \emph{scheduler-level} prefix-aware task ordering within groups to maximise the cache hit rate (Chapter~\ref{ch:future}).
...
This produces $k$ model groups $\{G_1, \ldots, G_k\}$ where each $G_i$ contains all tasks for one model, reducing switches from $O(n)$ to $O(k)$. Within each group, tasks are submitted to vLLM, which executes them via continuous batching: as one task completes, the next is admitted immediately without GPU idle time. A semaphore caps the number of concurrently queued tasks (default 10) to prevent the task queue from overwhelming GPU memory while keeping the engine fully utilised.
...
Four scheduling strategies are compared. FIFO executes tasks in submission order. Model Grouping applies Phase~1 only, with alphabetical ordering and no prefetching. Grouping + Prefetch applies Phase~1 grouping and enables checkpoint prefetching, but uses alphabetical group ordering rather than Johnson's Rule, serving as an intermediate ablation baseline. Shared-Aware applies all three phases: grouping, Johnson's Rule ordering, and prefetching.
...
Four falsifiable hypotheses are tested across five experiments.

\textbf{H1} (\textit{Model grouping dominates}): model-grouped scheduling reduces makespan by at least 80\% over FIFO by eliminating redundant model switches from $O(n)$ to $O(k)$.

\textbf{H2} (\textit{Prefetching hides I/O}): checkpoint prefetching reduces the visible model-loading penalty by approximately 50\%, because I/O and compute proceed in parallel rather than sequentially.

\textbf{H3} (\textit{Johnson's Rule beats naive ordering}): Johnson's Rule outperforms alphabetical and size-descending orderings by at least 5\% on mixed-model batches by placing compute-heavy groups first to maximise prefetch overlap.

\textbf{H4} (\textit{Gains are robust across scale}): makespan improvements persist as batch size grows from 99 to 501 tasks, confirming that scheduling overhead does not eliminate the benefit.
...
\textbf{Models Tested.}
All experiments use three Qwen models spanning a wide range of checkpoint sizes: Qwen3-0.6B ($\sim$1.2\,GB, $a=0.4$\,s), Qwen3-8B ($\sim$15.4\,GB, $a=5.1$\,s), and Qwen3-32B ($\sim$65\,GB, $a=21.7$\,s), where $a$ is the NVMe SSD load time. The 0.6B and 8B models each run on a single GPU; the 32B model exceeds single-GPU VRAM (24\,GB) and runs with tensor parallelism across 4 GPUs (TP=4), which the scheduler detects and allocates automatically.

\textbf{Baseline Strategies.}
...
A 501-task batch with three models (0.6B, 8B, 32B; 167 tasks each) was submitted in round-robin interleaved order. FIFO makespan is obtained from direct cold-start measurements: each model was run as an isolated single-task batch (full page-cache eviction and GPU release before each task) over 3 runs, yielding per-model cold-start times of 0.6B: $33.1\pm1.7$s, 8B: $64.8\pm5.5$s, 32B: $174.5\pm36.9$s. Since FIFO with $n=501$ interleaved tasks incurs one cold-start per task switch, the equivalent FIFO makespan is $167\times(33.1+64.8+174.5)=45{,}495\pm3{,}353$s ($\approx$12.6h); executing this in real time is impractical, so the measured per-switch costs are used to compute the equivalent FIFO runtime. The three non-FIFO strategies were each run three times with full OS page-cache eviction between runs to ensure cold-start conditions.
...
\caption{Incremental scheduling strategy ablation for 501 tasks with 3 models (0.6B, 8B, 32B). Each column adds one optimisation component.}
\label{fig:exp1_bar}
\end{figure}

\begin{table}[htbp]
\centering
\caption{Experiment 1: Scheduling strategy ablation (501 tasks, 3 models: 0.6B, 8B, 32B; 167 tasks each). FIFO makespan is computed from direct cold-start measurements (each model sampled over 3 runs); the equivalent full-scale runtime is $\approx$12.6 hours.}
\label{tab:exp1_results}
\begin{tabular}{lrrrr}
\toprule
Strategy & Makespan (s) & Throughput (req/s) & Switches & Speedup \\
\midrule
FIFO (computed) & 45,495 $\pm$ 3,353 & 0.011 & 500 & 1$\times$ \\
...
where $\bar{L}_{infer} = 0.67$s is the per-task average inference time. The 32B cold-start exhibited substantial variance ($\pm$36.9\,s) due to TP=4 vLLM initialisation variability, contributing the $\pm$3{,}353\,s uncertainty in the FIFO extrapolation. Model loading accounts for over 99\% of the FIFO execution time. Model Grouping reduces switches from 500 to 2, saving approximately 91s per avoided switch. Prefetching further reduces makespan by 9.5\% by hiding the SSD-to-RAM checkpoint transfer during the preceding group's compute phase; with 167 tasks per group the large compute windows allow SSD reads to complete before each transition, reducing the visible gap toward the irreducible lower bound characterised in Experiment~2. Johnson's Rule adds a further 20.4\%: reordering groups as 0.6B $\to$ 8B $\to$ 32B ensures that the longest prefetch window covers the 32B checkpoint transfer, and that the batch starts serving tasks immediately from the smallest model rather than waiting for the 32B cold load. H1 is confirmed: model grouping achieves over 99\% makespan reduction versus FIFO, and the three optimisations compose to a 276$\times$ speedup.
...
\begin{table}[htbp]
\centering
\caption{Experiment 2: Checkpoint prefetching (90 tasks, 3 models: 0.6B, 8B, 32B; 30 each). Per-transition gap is the measured idle time between consecutive groups; latency hidden is computed from the average transition time.}
\label{tab:exp2_results}
\begin{tabular}{lrrrr}
\toprule
Configuration & Makespan (s) & Trans 1: 0.6B$\to$8B (s) & Trans 2: 8B$\to$32B (s) & Latency Hidden \\
\midrule
No Prefetch  & 221.5 $\pm$ 3.5 & 61.9 & 115.2 & 0\% \\
With Prefetch & 140.3 $\pm$ 2.4 & 36.9 & 51.9  & 49.9\% \\
\bottomrule
\end{tabular}
\end{table}
...
\end{tikzpicture}
\caption{Illustrative execution timeline: without prefetch the GPU idles during model loading; with prefetch, loading overlaps with the preceding group's computation, hiding most of the transition cost.}
\label{fig:prefetch_timeline}
\end{figure}

\subsection{Analysis}

Prefetching reduced makespan by 36.7\% (221.5s to 140.3s), saving 81.2 seconds across two model transitions. As shown in Table~\ref{tab:exp2_results}, both per-transition gaps roughly halved: 61.9s$\to$36.9s (0.6B$\to$8B) and 115.2s$\to$51.9s (8B$\to$32B), hiding 49.9\% of loading latency overall. When a group begins execution, the scheduler triggers an asynchronous \texttt{prefetch\_to\_cpu} call via the gRPC \texttt{LoadModelAsync} endpoint; sllm-store loads checkpoint files from SSD into the pinned memory pool using multi-threaded I/O \citep{cuda_guide2024}, running in a background thread while the GPU executes the current group. At transition, weights are already in pinned memory, enabling direct DMA at near-peak PCIe bandwidth ($\sim$25\,GB/s). The residual transition time covers GPU memory allocation and CPU-to-GPU weight transfer, which cannot be overlapped with computation; 36.9s for the 8B model and 51.9s for the 32B model represent irreducible lower bounds.
...
Batch sizes of n=99 and n=501 were measured with three models (Qwen3-0.6B, Qwen3-8B, Qwen3-32B) in equal proportion (n/3 tasks each). A single projected data point at n=1,000 is included for illustration, derived from closed-form scaling equations calibrated at the two measured anchor points. Table~\ref{tab:exp3_scale} and Figure~\ref{fig:exp3_scalability} show the results.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\textwidth]{figure_exp5_scalability.pdf}
\caption{Makespan scalability across batch sizes (99--1,000 tasks, 3 models, log-log axes). FIFO grows linearly ($O(n)$ switches); Grouping and Shared-Aware incur only 2 fixed transitions. Solid lines are calibrated model fits over the measured range; dashed lines are projections to $n=1{,}000$. Error bars show $\pm 1$ std.}
...
\midrule
\multicolumn{6}{p{0.92\textwidth}}{\textit{Projected ($C_\text{FIFO}(n)\approx90.3n$, $C_G(n)\approx218+0.021n$, valid near calibrated range $n=99$--$501$):}} \\
\midrule
1,000 & 90,300$^\dagger$ & 25.1 h & 239$^\dagger$ & 200$^\dagger$ & 452$\times$ \\
\bottomrule
\multicolumn{6}{p{0.92\textwidth}}{$^\ddagger$FIFO computed from direct cold-start measurements (90.3s/switch average); equivalent full-scale runtime $\approx$2.5--12.6h. $^\dagger$Projected from $C(n)$ equations; SA from $C_{SA}(n)\approx129+0.071n$.} \\
\end{tabular}
\normalsize
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Experiment 4: All $3!=6$ group orderings ranked by mean makespan (90 tasks, 3 models, prefetch enabled, 3 runs each). Orderings naturally cluster into three tiers by first-position model.}
\label{tab:exp4_ordering_results}
\small
\begin{tabular}{lrrrl}
\toprule
Ordering & Makespan (s) & Rank & $\Delta$ JR \\
\midrule
\textbf{JR: 0.6B$\to$8B$\to$32B} & \textbf{132.4 $\pm$ 3.5} & \textbf{1/6} & \textbf{---} \\
0.6B$\to$32B$\to$8B           & 132.9 $\pm$ 4.4 & 2/6 & +0.4\% \\
8B$\to$32B$\to$0.6B           & 173.0 $\pm$ 2.8 & 3/6 & +30.7\%  \\
8B$\to$0.6B$\to$32B           & 175.7 $\pm$ 8.1 & 4/6 & +32.7\%  \\
32B$\to$8B$\to$0.6B           & 223.1 $\pm$ 8.7 & 5/6 & +68.5\% \\
32B$\to$0.6B$\to$8B           & 227.8 $\pm$ 5.1 & 6/6 & +72.1\% \\
\bottomrule
\end{tabular}
\normalsize
\end{table}
...
\end{tikzpicture}
\caption{Schematic execution timeline: worst-case (32B-first, top) vs Johnson's Rule (0.6B-first, bottom) ordering. Placing the largest model first forces an unmasked GPU idle at $t=0$; placing the smallest model first allows prefetching to begin immediately, reducing the residual gap.}
...
Johnson's Rule (0.6B$\to$8B$\to$32B) achieves rank~1/6, matching the empirical optimum exactly. With all three groups S1, JR orders by ascending SSD-load time, deterministically placing the smallest-footprint model first without exhaustive enumeration, and avoiding the 31--72\% penalties of non-0.6B-first orderings.
...
At $n=30$, the two 0.6B-first orderings tied because the compute window of the first group was wide enough to fully hide both subsequent prefetches: the combined SSD transfer for 8B and 32B ($\approx$26.8s) completes well before the 0.6B group finishes ($\approx$45s), so whichever of 8B and 32B runs second is already staged in pinned memory. The tie is a \emph{saturation} effect: once prefetching is fully hidden, the secondary ordering within the first tier no longer matters.

This raises a natural question: does the secondary ordering become meaningful when task count is reduced and the compute window shrinks? At $n=5$ tasks per model, each group runs for only $\approx$2--3s of compute, shorter than the SSD load times of the 8B ($a=5.1$s) and 32B ($a=21.7$s) models. These two groups therefore become \emph{I/O-dominant}: loading takes longer than executing them. Johnson's Rule handles this by switching to its S2 rule (placing I/O-dominant groups last, sorted by compute time descending), which prescribes 0.6B$\to$32B$\to$8B for this regime.

Table~\ref{tab:exp4_sensitivity} shows the results across $n=5$, 15, and 30. At $n=5$, the JR ordering (0.6B$\to$32B$\to$8B) achieves rank~2/6, 4.7s behind the empirical fastest; this gap is in the predicted direction but does not reach statistical significance ($t\approx2.2$, $p>0.05$ with 4 degrees of freedom). At $n=15$, the gap between the two 0.6B-first orderings widens to 7.3s, consistent with a gradual transition. By $n=30$, all groups become compute-dominant again and the gap collapses to 0.5s.

\begin{table}[htbp]
\centering
\caption{Task-count sensitivity: makespan (mean\,$\pm$\,std, seconds) for all six group orderings at three batch sizes. Johnson's Rule selects a different optimal ordering depending on task count: at $n=5$, the 8B and 32B groups are I/O-dominant (load time exceeds compute time), so JR places them later; at $n\geq15$, all groups are compute-dominant and JR sorts by ascending load time instead. The JR-selected ordering for each column is shown in bold.}
\label{tab:exp4_sensitivity}
\begin{tabular}{lccc}
\toprule
Ordering & $n=5$/model & $n=15$/model & $n=30$/model \\
\midrule
\textit{JR predicts:} & \textit{0.6B$\to$32B$\to$8B} & \textit{0.6B$\to$8B$\to$32B} & \textit{0.6B$\to$8B$\to$32B} \\
\midrule
0.6B$\to$8B$\to$32B & 131.1$\pm$2.2 & 137.9$\pm$5.8 & \textbf{132.4$\pm$3.5} \\
\textbf{0.6B$\to$32B$\to$8B} & \textbf{135.8$\pm$3.0} & 145.2$\pm$11.6 & 132.9$\pm$4.4 \\
8B$\to$32B$\to$0.6B & 192.7$\pm$14.1 & --- & 173.0$\pm$2.8 \\
...
\bottomrule
\multicolumn{4}{l}{\footnotesize ``---'': not measured at $n=15$. Bold marks the JR-predicted ordering for that column.}
\end{tabular}
\end{table}
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{figure_exp5_storage_tier.pdf}
...
\textbf{RQ1: How to Discover and Exploit Shared Structure?}
...
\textbf{RQ2: Can the Problem Be Modelled as a Two-Machine Flow-Shop?}
...
\textbf{RQ3: How Do Strategies Compare?}
...
Four limitations apply. First, the single-model-per-worker assumption is suboptimal for LoRA adapters, which can share base models with sub-second swaps. Second, Johnson's Rule assumes deterministic processing times, but LLM inference times vary with output length; conservative model-size estimates are used instead. Third, when pinned memory is insufficient for a proactive prefetch, the memory-aware guard defers loading to transition time; sllm-store's C++ LRU policy then evicts stale models on demand, so the transition still completes correctly but the prefetch overlap benefit is lost. Fourth, the algorithm processes one batch at a time without multi-tenant fairness guarantees. Johnson's Rule delivers maximum benefit when models have strongly contrasting I/O-compute ratios; with a homogeneous catalogue (all similar sizes), any permutation yields nearly identical makespan and the ordering contribution is negligible.
...
Three engineering improvements would enable production-scale deployment: streaming JSONL output to object storage, deadline-aware autoscaling ($\text{workers} = \lceil n \cdot L_{avg} / T_{deadline} \rceil$), and multi-node data parallelism with distributed queues. Abstracting the inference engine interface would add support for SGLang \citep{zheng2024sglang} and TensorRT-LLM \citep{tensorrt_llm2024}. The multi-GPU scheduling algorithm has been implemented and validated in the experiments reported here, where the 32B model runs with TP=4 across 4 GPUs and is detected and allocated automatically; systematic evaluation of its behaviour across a wider range of GPU counts, TP degrees, and mixed-parallelism configurations remains as future work.
...
Five contributions underpin this work: a two-machine flow-shop formulation mapping model loading and GPU inference to Johnson's framework; a shared-aware algorithm that groups tasks by model and applies Johnson's Rule; a checkpoint prefetching mechanism built on the sllm-store pinned memory pool; an evaluation of prefetching effectiveness across four storage tiers (SSD, RAID-0, and both 10 and 100\,GbE NFS); and an OpenAI-compatible batch API extended for multi-model batches.
