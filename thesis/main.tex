...
Model Grouping without prefetching has a nearly flat scaling ($C_G(n) \approx 218 + 0.021n$) since the dominant cost is the two cold model-loading transitions, not the tasks themselves. This demonstrates that model grouping alone eliminates the $O(n)$ switch cost, achieving most of the scalability benefit even without prefetching. Figure~\ref{fig:exp3_scalability} shows the Model Grouping and Shared-Aware lines converging as $n$ grows; this is a calibration artefact of fitting two slopes over the narrow range $n=99$--$501$. Physically, both strategies share the same per-task inference time, so their asymptotic slopes must be equal; the apparent convergence would reverse beyond the measured range. The meaningful comparison is the fixed-cost difference ($218$s vs $129$s), which reflects the prefetching benefit and is independent of $n$.
