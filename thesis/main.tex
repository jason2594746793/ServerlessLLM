...
The tie between the two 0.6B-first orderings arises because, at $n=30$ tasks per model, the 0.6B group's compute window ($\approx$60s) exceeds the SSD-to-CPU transfer time for both the 8B checkpoint (5.1s) and the 32B checkpoint (21.7s). Both subsequent models are fully staged in CPU memory before the 0.6B group finishes, so their relative order cannot affect makespan. To verify that JR's S2 sub-ranking is physically meaningful at shorter task counts, all six permutations were re-run at $n=5$ tasks per model, and the two 0.6B-first orderings were additionally measured at $n=15$.

At $n=5$, each model group executes only $\approx$5 tasks ($\approx$2--3s compute); the dominant cost per group is vLLM initialization (0.6B: $\approx$30s, 8B: $\approx$40s, 32B: $\approx$60s). In the 0.6B$\to$8B$\to$32B ordering, the two shorter initializations are sequenced first, allowing SSD prefetch of the 32B checkpoint to complete in the background during 8B's $\approx$40s initialization window; in 0.6B$\to$32B$\to$8B, the 60s 32B initialization appears earlier in the pipeline. JR correctly predicts 0.6B$\to$8B$\to$32B at this regime (all groups S1, ascending $a_i$).

Table~\ref{tab:exp4_sensitivity} shows the measured results. At $n=5$, the JR ordering (0.6B$\to$8B$\to$32B) achieves 131.1\,$\pm$\,2.2s, ranking 1st among all six permutations. The alternative 0.6B$\to$32B$\to$8B --- JR-optimal at $n=30$ --- places 2nd at 135.8\,$\pm$\,3.0s (+3.5\%, a 4.7s gap in the predicted direction). At $n=15$, the two 0.6B-first orderings measure 137.9\,$\pm$\,5.8s and 145.2\,$\pm$\,11.6s respectively, a 7.3s gap consistent with an intermediate regime. At $n=30$, the gap collapses to 0.5s within noise (Table~\ref{tab:exp4_ordering_results}).
...
\midrule
\textbf{0.6B$\to$8B$\to$32B} $\dagger$ & \textbf{131.1$\pm$2.2} & 137.9$\pm$5.8 & 132.4$\pm$3.5 \\
0.6B$\to$32B$\to$8B $\ddagger$ & 135.8$\pm$3.0 & 145.2$\pm$11.6 & \textbf{132.9$\pm$0.0} \\
...
\bottomrule
\end{tabular}
\footnotesize{$\dagger$\,JR-optimal at $n=5$ (all S1: ascending $a_i$); $\ddagger$\,JR-optimal at $n\geq15$ (mixed regime). ``---'': not measured at $n=15$ (consistent tier structure verified at $n=5$ and $n=30$).}
\end{table}

JR's ordering rule adapts correctly to workload regime: it prescribes 0.6B$\to$8B$\to$32B when all groups are compute-dominant (small $n$, all S1) and achieves rank~1/6; at larger $n$, it correctly shifts to 0.6B$\to$32B$\to$8B (mixed S1/S2) and achieves rank~2/6 within measurement noise of rank~1. In both regimes the three-tier structure (0.6B-first $\ll$ 8B-first $\ll$ 32B-first) is the dominant pattern, and JR deterministically selects the correct first-position model without runtime enumeration. At $n=5$, the worst ordering (32B$\to$0.6B$\to$8B, 219.4s) is 67\% slower than JR; the 88s spread across all six orderings confirms that group order has first-order impact regardless of task count.
