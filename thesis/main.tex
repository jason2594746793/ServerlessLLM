...
\begin{enumerate}
  \item \textbf{Remote storage $\to$ Local SSD} (optional): When models are not locally cached, checkpoint files are downloaded from object stores (S3, GCS) or network filesystems (NFS). Bandwidth: 1--12\,GB/s depending on network tier (see Section~\ref{sec:bg_storage_hierarchy}).
