...
\begin{itemize}
  \item \textbf{Two-machine flow-shop formulation.} The batch scheduling problem is formalised with Machine~A as I/O (loading model weights) and Machine~B as GPU computation, enabling Johnson's Rule for provably optimal model group ordering.
  \item \textbf{Shared-aware scheduling algorithm.} A scheduler discovers model-sharing relationships within a batch, groups tasks by model to reduce switches from $O(n)$ to $O(k)$, and orders groups by Johnson's Rule to maximise I/O-computation overlap.
  \item \textbf{Checkpoint prefetching mechanism.} Using the pinned memory pool of ServerlessLLM, the next model's weights are preloaded while the current group executes on GPU, hiding I/O latency through pipelining.
  \item \textbf{Storage tier analysis and evaluation.} The prefetch architecture is analysed and evaluated across storage tiers including RAID-0, 10\,GbE NFS, and 100\,GbE NFS, demonstrating that it generalises to network-attached storage.
  \item \textbf{OpenAI-compatible batch API.} A multi-model batch REST API enables clients to submit heterogeneous task lists in a single request and receive results asynchronously, following the OpenAI Batch API specification.
\end{itemize}
