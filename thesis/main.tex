...
Model grouping exploits the model-sharing structure formalised in Chapter~4 (Section~\ref{sec:problem_definition}), reducing switches from $O(n)$ to $O(k)$ for a 99\% makespan reduction (Experiment~1). The advantage grows with batch size as larger batches amortise the two fixed model-loading transitions over more tasks (Experiment~3).
